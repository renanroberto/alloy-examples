...
/* Alternative to one and appended fact
sig Root extends Dir {}
fact { one Root }
fact { all d : Root | no d.parent }
*/
...
assert fileHasOneDirectory {all f : File | one d : Dir | f in d.contents and f.parent = d}
check fileHasOneDirectory for 5

assert everyObjectHasCommonAncestor {
	all o, p : (FSObject - Root) | some d : Dir | d in o.^parent and d in p.^parent
}
check everyObjectHasCommonAncestor for 5
